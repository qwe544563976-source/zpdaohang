...
**V3 查询内核（2026-08-21，老板用真实考卷定案）**：

- 支持路精准查询由生成器从 claim_terms 逐字短引**确定性派生**（`derive_step_queries`：每条条件短引一条、每条结果短引一条、条件＋结果组合、梵英双轨变体），绑定 step_id，模型不再自由发挥；验证器用同一共享函数独立重派生并逐字节比对。
- 模型写的英文查询过**逐字闸门**（`is_verbatim_in`：整句在冻结原文＋章名里连续命中，或 ≤3 段、每段 ≥4 个连续词，单个梵文专名放行）。自验（--check-only）零容忍；正式装配机械剔除并记入 `query_verbatim_findings`。
- **空车道合法**：反例／边界／方法路原文没有就留空数组，禁止硬塞落陷／燃烧／D9／化解；支持路要求抽取员查询或派生查询至少一路非空。
- 通用与书籍分离：派生函数是通用内核；宫名序数表、行星英译表是本书专属（`BHAVA_ORDINALS`／`GRAHA_ENGLISH`），换书换表不换函数。
- retrieval_plan 在方法层、不进 step_hash——查询升级不作废审计凭证。

**批量工单纪律**：批次组状态（哪组已抽、哪组缺）一律由 `distillation/prepare_batches.py` 从目录实况生成，**任何人不得手写**；抽取 agent 遇工单与实况矛盾，以实况为准并报告。修理单只可信机器报错原文，先复现再动手。agent 返回必须原样抄机器自验结果（check_passed），口头完成无效。
