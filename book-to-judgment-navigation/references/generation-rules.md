...
### 四路查询自由发挥（V2 丢分根因，2026-08-21 老板用真实考卷定案，V3 起机器接管）

真实丢分样本（老板的 20 题考卷）：`Dharm Lord in Kendras` 宽词漏掉 v106 的
「十宫见王」专句；`Dhan Lord in Dusthanas` 漏掉 v24 的十二宫破财专句；
`Budh Dasha wealth gain` 丢掉小运主星 Rahu。根因：查询靠模型看完方法后自由发挥。

V3 处理（全部机器化，模型不再自由发挥）：
- 支持路精准查询由 `derive_step_queries()` 从逐字短引确定性派生（步级靶向、
  绑定 step_id、方向天然正确、梵英双轨），生成器与验证器各自独立执行同一函数比对；
- 模型写的英文查询过逐字闸门 `is_verbatim_in()`（整句连续命中，或 ≤3 段每段 ≥4 词，
  单个专名放行）——宽泛意译、编造的 D9/落陷/化解、词序拼盘一律现形。
  自验（--check-only）判不通过；正式装配机械剔除并记入
  `query_verbatim_findings`（第 1、2 批实测剔除 317 条 V2 老式拼盘，
  规则收紧后抽的三批零剔除）；
- 空车道合法化：反例/边界/方法路允许空数组（原文没有就不硬凑），
  支持路要求"抽取员查询或派生查询至少有一路非空"。

边界如实声明：方向颠倒若恰好对应书里另一条真实规则（十落九也真有其文），
逐字闸门拦不住——但支持路已由派生接管（方向由短引决定，天然正确），
残余风险只在模型写的反例/边界路，归审计盯。
