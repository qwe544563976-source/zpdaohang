---
method: ch12-v12-14-third-decanate-limb-order
...
- 本盘一宫（Lagn）是否落在所在星座的第三个三分盘（Drekkana D3）
...
### ch12-v12-14-third-decanate-limb-order.step-001

- 动作：核对上升是否落在所在星座的第三个三分盘（Drekkana D3），若是则取这一组宫位与肢体的对应。
- 适用范围：仅限本命盘上升三分盘（Drekkana D3）与身体部位的对应；原文只给出肢体次序，没有说这一串从哪一宫起算。
- 原文最小意思：上升落在星座的第三个三分盘（third decanate）时，各宫依次代表骨盆、肛门/阴茎、睾丸、大腿、膝、小腿和足。
- 本步骤产出事实：["第三个三分盘上升时的宫位肢体对应"]
- 所需事实：["本盘一宫（Lagn）是否落在所在星座的第三个三分盘（Drekkana D3）"]
- 条件关系：{"fact_key": "本盘一宫（Lagn）是否落在所在星座的第三个三分盘（Drekkana D3）"}
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["不得把这一组肢体次序用在其他三分盘上升的盘上。", "不得自行指定这一串肢体从哪一宫起算，原文没有写。", "不得据此直接断出疾病或吉凶，本偈只给对应关系。"]
- 来源状态：`single_explicit_source`
- 停止条件：缺少以下固定事实即停止：本盘一宫（Lagn）是否落在所在星座的第三个三分盘（Drekkana D3）。
- 缺失即停字段：["本盘一宫（Lagn）是否落在所在星座的第三个三分盘（Drekkana D3）"]
- 原文证据：
  - `bphs-97:santhanam:ch12:v12-14`｜PDF [27]｜“The order for the third decanate ascending is pelvis, anus/penis, testicles, thigh, knee, calf and foot.”
