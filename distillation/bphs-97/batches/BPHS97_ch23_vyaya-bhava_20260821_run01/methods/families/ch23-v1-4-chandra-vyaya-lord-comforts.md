...
- 动作：先确认十二宫（Vyaya）的宫主是不是月亮（Chandra），再核对月亮是否入旺、是否落本宫或自己主管的九分盘（Navamsa D9）星座，以及是否在本盘或九分盘中落十一宫（Labh）、九宫（Dharm）、五宫（Putr）。
