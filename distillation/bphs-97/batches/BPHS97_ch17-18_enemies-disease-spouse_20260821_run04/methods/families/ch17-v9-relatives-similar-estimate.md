...
- 动作：换用所论亲属的指示星与对应宫位，照本段（9-12½ 疾病总论）的同一套估量方式推断该亲属。
- 适用范围：原文为方法性指示，举父亲为例；未说明可推到哪些亲属，也未给时间限定。「Similar」承接的是紧接其前的 9-12½ 疾病总论一段，原文没有把授权扩到整章，本章其余各段（如 13-19½ 的发病年龄）不在此授权范围内。
...
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["不得把本章其他段落的具体病症或发病年份搬到亲属身上；原文只授权照本段疾病总论的方式估量。", "不得据此把本人的结论直接当作亲属的结论。"]
- 来源状态：`single_explicit_source`
