---
method: ch24-v99-dharm-lord-in-sahaj
...
### ch24-v99-dharm-lord-in-sahaj.step-001

- 动作：核对九宫主（Dharm's Lord）是否落三宫（Sahaj Bhava），据此判断兄弟之乐与财富德行。
- 适用范围：仅限本命盘九宫主（Dharm's Lord）落三宫（Sahaj Bhava）一条；原文未给时间限定。
- 原文最小意思：九宫主（Dharm's Lord）落三宫（Sahaj Bhava）时，享有兄弟之乐，富有、有德、有魅力。
- 本步骤产出事实：["九宫主落三宫的兄弟之乐与财富德行判定"]
- 所需事实：["九宫主（Dharm's Lord）是否落三宫（Sahaj）"]
- 条件关系：{"fact_key": "九宫主（Dharm's Lord）是否落三宫（Sahaj）"}
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["本条只对应九宫主落三宫，不得套用到九宫主落其他宫位。", "不得把「兄弟之乐」扩大成兄弟人数、排行或姐妹。", "不得据此推断财富数额或成名时间。"]
- 来源状态：`single_explicit_source`
- 停止条件：缺少以下固定事实即停止：九宫主（Dharm's Lord）是否落三宫（Sahaj）。
- 缺失即停字段：["九宫主（Dharm's Lord）是否落三宫（Sahaj）"]
- 原文证据：
  - `bphs-97:santhanam:ch24:v99`｜PDF [47]｜“If Dharm’s Lord is in Sahaj Bhava, the native will be endowed with fraternal bliss, be wealthy, virtuous and charming.”
