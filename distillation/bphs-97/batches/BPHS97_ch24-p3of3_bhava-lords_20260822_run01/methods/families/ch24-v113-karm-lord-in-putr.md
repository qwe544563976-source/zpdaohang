---
method: ch24-v113-karm-lord-in-putr
...
### ch24-v113-karm-lord-in-putr.step-001

- 动作：核对十宫主（Karm's Lord）是否落五宫（Putr Bhava），据此判断各类学问与财富子嗣。
- 适用范围：仅限本命盘十宫主（Karm's Lord）落五宫（Putr Bhava）一条；原文未给时间限定。
- 原文最小意思：十宫主（Karm's Lord）落五宫（Putr Bhava）时，具备各类学问，常常欢喜，富有并且有子。
- 本步骤产出事实：["十宫主落五宫的各类学问与财富子嗣判定"]
- 所需事实：["十宫主（Karm's Lord）是否落五宫（Putr）"]
- 条件关系：{"fact_key": "十宫主（Karm's Lord）是否落五宫（Putr）"}
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["本条只对应十宫主落五宫，不得套用到十宫主落其他宫位。", "不得把「有子」写成具体子女人数或性别。", "不得把「各类学问」写成某个专业或学历。"]
- 来源状态：`single_explicit_source`
- 停止条件：缺少以下固定事实即停止：十宫主（Karm's Lord）是否落五宫（Putr）。
- 缺失即停字段：["十宫主（Karm's Lord）是否落五宫（Putr）"]
- 原文证据：
  - `bphs-97:santhanam:ch24:v113`｜PDF [48]｜“If Karm’s Lord is in Putr Bhava, the native will be endowed with all kinds of learning, he will be always delighted and he will be wealthy and endowed with sons.”
