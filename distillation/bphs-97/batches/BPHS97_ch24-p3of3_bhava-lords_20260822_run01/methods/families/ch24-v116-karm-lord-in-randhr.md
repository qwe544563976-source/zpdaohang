---
method: ch24-v116-karm-lord-in-randhr
...
### ch24-v116-karm-lord-in-randhr.step-001

- 动作：核对十宫主（Karm's Lord）是否落八宫（Randhr Bhava），据此判断少有作为与长寿。
- 适用范围：仅限本命盘十宫主（Karm's Lord）落八宫（Randhr Bhava）一条；原文未给时间限定。
- 原文最小意思：十宫主（Karm's Lord）落八宫（Randhr Bhava）时，没有作为，长寿，一心指责他人。
- 本步骤产出事实：["十宫主落八宫的少有作为与长寿判定"]
- 所需事实：["十宫主（Karm's Lord）是否落八宫（Randhr）"]
- 条件关系：{"fact_key": "十宫主（Karm's Lord）是否落八宫（Randhr）"}
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["本条只对应十宫主落八宫，不得套用到十宫主落其他宫位。", "不得把「长寿」换算成具体岁数或寿限区间。", "不得把「没有作为」写成失业、破产或某个具体事件。"]
- 来源状态：`single_explicit_source`
- 停止条件：缺少以下固定事实即停止：十宫主（Karm's Lord）是否落八宫（Randhr）。
- 缺失即停字段：["十宫主（Karm's Lord）是否落八宫（Randhr）"]
- 原文证据：
  - `bphs-97:santhanam:ch24:v116`｜PDF [48]｜“If Karm’s Lord is in Randhr Bhava, the native will be devoid of acts, long-lived and intent on blaming others.”
