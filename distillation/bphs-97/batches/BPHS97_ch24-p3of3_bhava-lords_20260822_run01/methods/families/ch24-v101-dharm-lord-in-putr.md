---
method: ch24-v101-dharm-lord-in-putr
...
### ch24-v101-dharm-lord-in-putr.step-001

- 动作：核对九宫主（Dharm's Lord）是否落五宫（Putr Bhava），据此判断子嗣兴旺与品行学问。
- 适用范围：仅限本命盘九宫主（Dharm's Lord）落五宫（Putr Bhava）一条；原文未给时间限定。
- 原文最小意思：九宫主（Dharm's Lord）落五宫（Putr Bhava）时，有子并且兴旺，敬奉尊长、胆大、乐善好施、有学问。
- 本步骤产出事实：["九宫主落五宫的子嗣兴旺与品行学问判定"]
- 所需事实：["九宫主（Dharm's Lord）是否落五宫（Putr）"]
- 条件关系：{"fact_key": "九宫主（Dharm's Lord）是否落五宫（Putr）"}
- 按分支必查事实：[]
- 例外、取消或缓解：[]
- 禁止扩大：["本条只对应九宫主落五宫，不得套用到九宫主落其他宫位。", "不得把「有子」写成具体子女人数或性别比例。", "不得据此推断求学阶段或功名时间。"]
- 来源状态：`single_explicit_source`
- 停止条件：缺少以下固定事实即停止：九宫主（Dharm's Lord）是否落五宫（Putr）。
- 缺失即停字段：["九宫主（Dharm's Lord）是否落五宫（Putr）"]
- 原文证据：
  - `bphs-97:santhanam:ch24:v101`｜PDF [47]｜“If Dharm’s Lord is in Putr Bhava, the native will be endowed with sons and prosperity, devoted to elders, bold, charitable and learned.”
